...
class TestVisits extends BaseLoggedClass {
    ...
    private static final String VISIT_DATE = "2023 Mar 15";
    ...
    @Test
    @DisplayName("AddVisitToPet")
    void addVisitToPet() throws ElementNotFoundException {
        ...
        List<WebElement> visitRows = driver.findElements(By.cssSelector("table.table-condensed tr"));
        Assertions.assertTrue(
                ...
                "No visit row found with date '" + VISIT_DATE + "' and description '" + VISIT_DESCRIPTION + "'"
                        + " — actual rows: " + visitRows.stream().map(WebElement::getText).collect(Collectors.toList()));
    }
}